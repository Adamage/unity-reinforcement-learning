...
public abstract class Academy : MonoBehaviour
{
	...
	* If true, all agents done flags will be set to true.*/
    ...
    /** Increments each time a step is taken in the environment. Is
    * reset to 0 during AcademyReset(). */

    public Communicator communicator;
    /** Do not modify : pointer to the communicator currently in 
	 * use by the Academy. */

    [HideInInspector]
    public bool isInference;
    /** Do not modify : If true, the Academy will use inference 
	 * settings. */

    [HideInInspector]
    public bool windowResize;
    /** Do not modify : Used to determine if the application window 
